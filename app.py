...
if search_query:
    filtered_df = filtered_df[
        ...
        | filtered_df["Problem Statement Title"].str.contains(
...
if filtered_df.empty:
    st.warning(
        "No problem statements match the selected filters or search query. Please adjust your search."
    )
    st.stop()
...
with col2:
    st.markdown(
        f"<p style='text-align: center; font-size: 18px;'>Problem {current_index + 1} of {total_problems}</p>",
...
new_status = st.selectbox(
    "Update Status",
    ["Not Reviewed", "Shortlisted", "Rejected", "See Later"],
    index=["Not Reviewed", "Shortlisted", "Rejected", "See Later"].index(
        current_status
    ),
)
...
new_notes = st.text_area(
    "Write your notes here:", value=current_notes, height=120, key=notes_key
)
...
# Show the prompt in a code block for easy manual copy
st.markdown("#### 📋 AI Prompt (click to copy below)")
st.code(prompt_json, language="json")

# Add a JS-powered copy button (works on Streamlit Cloud)

# Prepare the prompt for JS (escape backticks and backslashes)
# prompt_json_js = prompt_json.replace("\\", "\\\\").replace("`", "\\`")
...
#   navigator.clipboard.writeText(`{prompt_json_js}`);
